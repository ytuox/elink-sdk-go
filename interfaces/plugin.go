...
type Plugin interface {
	// PluginNotify 插件状态更新通知
	PluginNotify(ctx context.Context, t common.PluginNotifyType, name string) error
	// DeviceNotify 设备增删改通知,删除设备时device参数为空
	DeviceNotify(ctx context.Context, t common.DeviceNotifyType, deviceId string, device model.Device) error
	// ProductNotify 产品增删改通知,删除产品时product参数为空
	ProductNotify(ctx context.Context, t common.ProductNotifyType, productId string, product model.Product) error
